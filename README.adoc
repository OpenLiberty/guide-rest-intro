...
After you are done checking out the application, simply press `CTRL+C` in the shell session where
you ran the server, or run the `liberty:stop` goal from the `finish` directory in
...
the Maven [hotspot=defaultHttpPort hotspot=defaultHttpsPort file=1]`pom.xml` file. This is specified in the [hotspot=httpEndpoint file=0]`<httpEndpoint/>` element. Variables use the syntax `${variableName}`.
...
The variables being used in the [hotspot=httpEndpoint hotspot=webApplication file=0]`server.xml` file are provided by the properties set in the Maven [hotspot=defaultHttpPort hotspot=defaultHttpsPort hotspot=appContextRoot file=1]`pom.xml` file. The properties must be formated as `liberty.var.variableName`.

== Building and running the application

The Open Liberty server was already started in development mode at the beginning of the guide and all the 
...
Since we started Open Liberty in development mode at the start of the guide, simply press
...
When you are done checking out the service, exit development mode by simply typing `q` in the shell session where

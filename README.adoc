...
[role="code_command hotspot=1-34"]
----
...
The [hotspot=11-12]`@Path` annotation on the class indicates that this resource responds to the `properties` path
...
The [hotspot=13-14]`@GET` annotation on the method indicates that this method is to be called for the HTTP `GET`
method. The [hotspot=14-15]`@Produces` annotation indicates the format of the content that will be returned, the
...
. Creates a [hotspot=17-18]`JsonObjectBuilder` object using the `Json` class. The `JsonObjectBuilder` is then used
to populate a `JsonObject` with values.
. Calls the [hotspot=19-20]`getProperties` method on the `System` class to get a `Properties` object that contains
all the system properties.
. Calls the [hotspot=21-22]`entrySet` method on the `Properties` object to get a `Set` of all the entries.
. Convert the [hotspot=21-22]`Set` to a `Stream` (new in Java SE 8) by calling the `stream` method. Streams make
...
. Returns the [hotspot=25-26]`JsonObject` by calling the `build` method on the `JsonObjectBuilder`.
...
`pom.xml` file. This is specified in the [hotspot=10-11]`webApplication` element.
...
[role="code_command hotspot=1-43"]
----
...
The test method is indicated with the [hotspot=15-16]`@Test` annotation.
...
representation. To get the JAX-RS client to do the conversion, the client needs to have the [hotspot=22-23]`JsrJsonpProvider`

...
It will pass the `Throwable` to the [hotpost=toResponse file=1]`toResponse()` method to construct a response.
The [hotspot file=0]`ErrorResponse` class defines the JSON contents of the response. 

The `GenericExceptionMapper` class will be used for any `Throwable` that does not have a more specific `ExceptionMapper` interface implemented. 

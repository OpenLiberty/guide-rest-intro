...
To get the service running, the `server.xml` needs to be parameterized with the required variables to configure the RESTful web service that runs on the Liberty server. After declaring the configured variables in the [hotspot=httpEndpoint hotspot=webApplication file=0]`server.xml` file, they are then defined and passed through the properties set in the [hotspot=defaultHttpPort hotspot=defaultHttpsPort hotspot=appContextRoot file=1]`pom.xml` file. The properties must be formatted as `liberty.var.variableName`.
...
. Configures the server to resolve the HTTP port numbers from variables, which is specified in the [hotspot=httpEndpoint file=0]`<httpEndpoint/>` element. The `httpEndpoint` is then fed through the [hotspot=defaultHttpPort hotspot=defaultHttpsPort file=1]`pom.xml` file as a property which acts as a value placeholder. Variables use the syntax`${variableName}`.
. Configures the server to run the produced web application on a context root, specified in the [hotspot=webApplication file=0]`<webApplication/>` element. The [hotspot=appContextRoot file=1]`</appContextRoot>` is fed through the `pom.xml` as property to also act as a value placeholder. 

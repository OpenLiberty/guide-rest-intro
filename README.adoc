...
the Maven hot`pom.xml` file. This is specified in the [hotspot=8-9 file=0]`httpEndpoint` element. Variables use the syntax `${variableName}`.
. Configures the server to run the produced Web application on a context root specified in the Maven
[hotspot=1=203 file=1]`pom.xml` file. This is specified in the [hotspot=11 file=0]`webApplication` element.

The variables being used in the [hotspot=1-12 file=0]`server.xml` file are provided by the [hotspot=147-151 file=1]`<bootstrapProperties/>` section
of the Maven [hotspot=1-203 file=1]`pom.xml`:

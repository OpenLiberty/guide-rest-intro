...
. Configures the server to enable JAX-RS. This is specified in the [hotspot=3-6 file=0]`featureManager` element.
...
the [hotspot=25 file=0]`target` method providing the URL. To cause the HTTP request to occur [hotspot=26 file=0]`request().get()` 
is invoked on the `WebTarget`. The [hotspot=26 file=0]`get` method

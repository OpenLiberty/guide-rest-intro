...
the Maven [hotspot=defaultHttpPort,defaultHttpsPort file=1]`pom.xml` file. This is specified in the [hotspot=httpEndpoint file=0]`<httpEndpoint/>` element. Variables use the syntax `${variableName}`.
...
The variables being used in the [hotspot file=0]`server.xml` file are provided by the properties set in the Maven [hotspot=portNumbers,appContextRoot file=1]`pom.xml` file. The properties must be formated as `liberty.var.${variableName}`.
...
file. Refer to the [hotspot file=1]`pom.xml` file to see how the application information such as the [hotspot=appContextRoot file=1]`app.context.root`, [hotspot=defaultHttpPort file=1]`default.http.port` and [hotspot=defaultHttpsPort file=1]`default.https.port` elements are provided in the file.

...
After you are done checking out the application, stop the Open Liberty server simply press CTRL+C in the shell session or run following in another shell session:
...
the Maven [hotspot file=2]`bootstrap.properties` file. This is specified in the [hotspot=httpEndpoint file=0]`<httpEndpoint/>` element. Variables use the syntax `${variableName}`.
. Configures the server to run the produced Web application on a context root specified in the 
[hotspot file=1]`bootstrap.properties` file. This is specified in the [hotspot=webApplication file=0]`<webApplication/>` element.
...
The variables being used in the [hotspot file=0]`server.xml` file are provided by the properties set in the [hotspot file=1]`bootstrap.properties` file.

== Building and running the application

The Open Liberty server was already started in developer mode at the begginning of the guide and all the 
changes have been automatically picked up.
...
information can be hardcoded, it is better to specify it in a single place like the Maven [hotspot file=2]`bootstrap.properties`
file. Refer to the [hotspot file=2]`bootstrap.properties` file to see how the application information such as the [hotspot=appname file=2]`app.context.root`, [hotspot=httpport file=2]`default.http.port` and [hotspot=httpsport file=2]`default.https.port` elements are provided in the file.
...
Since we started Open Liberty in developer mode at the start of the guide, simply press
`enter` to run the tests. You will see the following output:
...
When you are done checking out the service, stop the Open Liberty server simply press `CTRL+C` 
in the shell session where you ran development mode, or run the `liberty:stop` goal from 
the `start` directory in another shell session.

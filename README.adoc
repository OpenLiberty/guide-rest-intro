...
[role="code_command hotspot=12-34", subs="quotes"]
----
...
by the annotations specified on the methods. In the application you are building, an HTTP [hotspot=14-33]`GET` request
...
value of the [hotspot=15]`@Produces` annotation will be specified in the HTTP `Content-Type` response header.
...
. Returns the `JsonObject` by calling the [hotspot=26]`build` method on the `JsonObjectBuilder`.
...
the Maven [hotspot=1-210 file=1]`pom.xml` file. This is specified in the [hotspot=8-9 file=0]`httpEndpoint` element. Variables use the syntax `${variableName}`.
. Configures the server to run the produced Web application on a context root specified in the Maven
[hotspot=1-210 file=1]`pom.xml` file. This is specified in the [hotspot=11 file=0]`webApplication` element.

The variables being used in the [hotspot=1-12 file=0]`server.xml` file are provided by the [hotspot=148-152 file=1]`<bootstrapProperties/>` section
of the Maven [hotspot=1-210 file=1]`pom.xml`:
...
[role="code_command hotspot=14-43", subs="quotes"]
----
...
information can be hardcoded, it is better to specify it in a single place like the Maven [hotspot=1-210 file=1]`pom.xml`
file. Open the [hotspot=1-210 file=1]`pom.xml` file to see how the application information such as the [hotspot=22 file=1]`<app.name/>`, [hotspot=23 file=1]`<testServerHttpPort/>` and [hotspot=24 file=1]`<testServerHttpsPort/>` elements are provided in the file.

These Maven properties are then passed to the Java test program as the [hotspot=181-185 file=1]`<systemPropertyVariables/>` element in
the [hotspot=1-210 file=1]`pom.xml` file.
...
on `WebTarget` and then the [hotspot=26 file=0]`get` method on the returned object need to be called. The `get` method
...
You developed a REST service by using JAX-RS, JSON-P, and in Open Liberty.

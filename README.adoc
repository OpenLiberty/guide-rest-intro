...
These Maven properties are then passed to the Java test program as a series of system properties in the `pom.xml` file:
...
Getting the values to create a representation of the URL is simple. The test class uses the properties to get the application details:

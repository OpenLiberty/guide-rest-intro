...
the [hotspot=11]`@Path` annotation in this class indicates that the resource is available at the `System/properties`
path.
...
The [hotspot=14]`@GET` annotation on the method indicates that this method is to be called for the HTTP [hotspot=14]`GET`
...
. Creates a [hotspot=18]`JsonObjectBuilder` object using the `Json` class. The [hotspot=18]`JsonObjectBuilder` is then used
...
the Maven `pom.xml` file. This is specified in the [hotspot=8-9 file=0]`httpEndpoint` element. Variables use the syntax `${variableName}`.
. Configures the server to run the produced Web application on a context root specified in the Maven
`pom.xml` file. This is specified in the [hotspot=11 file=0]`webApplication` element.
...
file. Refer to the [hotspot=1-210 file=1]`pom.xml` file to see how the application information such as the [hotspot=22 file=1]`<app.name/>`, [hotspot=23 file=1]`<testServerHttpPort/>` and [hotspot=24 file=1]`<testServerHttpsPort/>` elements are provided in the file.

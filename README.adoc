...
The variables being used in the [hotspot=httpEndpoint hotspot=webApplication file=0]`server.xml` file are provided by the properties set in the Maven [hotspot=portNumbers hotspot=appContextRoot file=1]`pom.xml` file. The properties must be formated as `liberty.var.${variableName}`.

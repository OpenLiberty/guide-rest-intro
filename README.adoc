...
After you are done checking out the application, stop the Open Liberty server by simply type `q` in the shell session where
...
When you are done checking out the service, stop the Open Liberty server by simply type `q` in the shell session where
you ran the server and then press the `enter/return` key.

...
and remote JVM would be the same. In this case, an [hotspot=assertosname file=0]`assertion` is made that the `os.name` system property
for both JVMs is the same. You could write additional assertions `assertEquals` to check for more values.
